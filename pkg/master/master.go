...
type ExtraConfig struct {
	...
	// Builder for the service account issuer metadata
	ServiceAccountIssuerMetadataBuilder serviceaccount.OpenIDMetadataBuilder
...
func (c completedConfig) New(delegationTarget genericapiserver.DelegationTarget) (*Master, error) {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.ServiceAccountIssuerDiscovery) {
		md, err := c.ExtraConfig.ServiceAccountIssuerMetadataBuilder.
			OpenIDMetadata(c.GenericConfig.ExternalAddress)
		if err != nil {
			...
			klog.Errorf("Could not construct pre-rendered responses for"+
				" ServiceAccountIssuerDiscovery endpoints. Endpoints will not be"+
				" enabled. Error: %v", err)
		} else {
			routes.NewOpenIDMetadataServer(md.MetadataJSON, md.PublicKeysetJSON).
