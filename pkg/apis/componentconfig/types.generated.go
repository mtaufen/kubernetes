...
func (x *KubeletConfiguration) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		yyv147 := &x.HostIPCSources
		yym148 := z.DecBinary()
		_ = yym148
		if false {
		} else {
			z.F.DecSliceStringX(yyv147, false, d)
		}
	}
	yyj120++
	if yyhl120 {
	...
	if r.TryDecodeAsNil() {
		...
		yyv166 := &x.StreamingConnectionIdleTimeout
		yym167 := z.DecBinary()
		_ = yym167
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv168 := &x.NodeStatusUpdateFrequency
		yym169 := z.DecBinary()
		_ = yym169
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv232 := &x.APIServerList
		yym233 := z.DecBinary()
		_ = yym233
		if false {
		} else {
			z.F.DecSliceStringX(yyv232, false, d)
		}
	}
	yyj120++
	if yyhl120 {
