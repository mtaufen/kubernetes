...
func (x *KubeletConfiguration) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		yyv133 := &x.HostNetworkSources
		yym134 := z.DecBinary()
		_ = yym134
		if false {
		} else {
			z.F.DecSliceStringX(yyv133, false, d)
		}
	}
	yyj110++
	if yyhl110 {
	...
	if r.TryDecodeAsNil() {
		...
		yyv156 := &x.StreamingConnectionIdleTimeout
		yym157 := z.DecBinary()
		_ = yym157
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv158 := &x.NodeStatusUpdateFrequency
		yym159 := z.DecBinary()
		_ = yym159
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv200 := &x.OutOfDiskTransitionFrequency
		yym201 := z.DecBinary()
		_ = yym201
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv210 := &x.EvictionPressureTransitionPeriod
		yym211 := z.DecBinary()
		_ = yym211
		if false {
