...
func validateNodeConfigStatus(status *core.NodeConfigStatus, fldPath *field.Path) field.ErrorList {
	...
	if status.Assigned != nil {
		// Assigned is a reflection of the spec, which acts as an ACK from the Kubelet regarding recognition,
		// of the assignment. As such, it is validated against the same rules as the spec, rather than status.
		allErrs = append(allErrs, validateNodeConfigSourceSpec(status.Assigned, fldPath.Child("assigned"))...)
	}
...
func validateConfigMapNodeConfigSourceStatus(source *core.ConfigMapNodeConfigSource, fldPath *field.Path) field.ErrorList {
	...
	if source.Namespace == "" {
		allErrs = append(allErrs, field.Required(fldPath.Child("namespace"), "namespace must be set in status"))
	}
	if source.KubeletConfigKey == "" {
		allErrs = append(allErrs, field.Required(fldPath.Child("kubeletConfigKey"), "kubeletConfigKey must be set in status"))
	}
	if string(source.UID) == "" {
