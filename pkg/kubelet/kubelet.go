...
type KubeletBuilder func(kc_old *KubeletConfig, kc_new *componentconfig.KubeletConfiguration) (KubeletBootstrap, error)
...
func makePodSourceConfig(kc_old *KubeletConfig, kc_new *componentconfig.KubeletConfiguration) (*config.PodConfig, error) {
	...
	if kc_new.Config != "" {
		glog.Infof("Adding manifest file: %v", kc_new.Config)
		config.NewSourceFile(kc_new.Config, kc_old.NodeName, kc_new.FileCheckFrequency.Duration, cfg.Channel(kubetypes.FileSource))
	}
...
func NewMainKubelet(kc_old *KubeletConfig, kc_new *componentconfig.KubeletConfiguration) (*Kubelet, error) {
	...
	thresholds, err := eviction.ParseThresholdConfig(kc_new.EvictionHard, kc_new.EvictionSoft, kc_new.EvictionSoftGracePeriod)
	if err != nil {
		return nil, err
	}
	...
	oomWatcher := NewOOMWatcher(kc_old.CAdvisorInterface, kc_old.Recorder)
	...
	case "docker":
		...
		klet.containerRuntime = dockertools.NewDockerManager(
			kc_old.DockerClient,
			kubecontainer.FilterEventRecorder(kc_old.Recorder),
	...
	klet.volumePluginMgr, err =
		NewInitializedVolumePluginMgr(klet, kc_old.VolumePlugins)
	if err != nil {
		return nil, err
	}
	...
	klet.podWorkers = newPodWorkers(klet.syncPod, kc_old.Recorder, klet.workQueue, klet.resyncInterval, backOffPeriod, klet.podCache)
	...
	evictionManager, evictionAdmitHandler, err := eviction.NewManager(klet.resourceAnalyzer, evictionConfig, killPodNow(klet.podWorkers), kc_old.Recorder, nodeRef, klet.clock)
	if err != nil {
	...
	for _, opt := range kc_old.Options {
		opt(klet)
	}
