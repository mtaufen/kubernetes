...
type NetworkPlugin interface {
	...
	Init(host Host, hairpinMode componentconfig.HairpinMode, nonMasqueradeCIDRs []string, mtu int) error
...
func InitNetworkPlugin(plugins []NetworkPlugin, networkPluginName string, host Host, hairpinMode componentconfig.HairpinMode, nonMasqueradeCIDRs []string, mtu int) (NetworkPlugin, error) {
	if networkPluginName == "" {
		// default to the no_op plugin
		plug := &NoopNetworkPlugin{}
		if err := plug.Init(host, hairpinMode, nonMasqueradeCIDRs, mtu); err != nil {
			return nil, err
		}
		return plug, nil
	}
	...
	if chosenPlugin != nil {
		err := chosenPlugin.Init(host, hairpinMode, nonMasqueradeCIDRs, mtu)
		if err != nil {
...
func (plugin *NoopNetworkPlugin) Init(host Host, hairpinMode componentconfig.HairpinMode, nonMasqueradeCIDRs []string, mtu int) error {
