...
// KubeletConfigController is the controller which, among other things:
...
type KubeletConfigController struct {
	...
	// initConfig is the unmarshaled init config, this will be loaded by the KubeletConfigController if an initConfigDir is provided
...
// NewKubeletConfigController constructs a new KubeletConfigController object and returns it. Directory paths must be absolute.
...
func NewKubeletConfigController(initConfigDir string, dynamicConfigDir string, defaultConfig *componentconfig.KubeletConfiguration) *KubeletConfigController {
	fs := utilfs.DefaultFs{}
	...
	return &KubeletConfigController{
...
// Bootstrap attempts to return a valid KubeletConfiguration based on the configuration of the KubeletConfigController,
...
func (cc *KubeletConfigController) Bootstrap() (*componentconfig.KubeletConfiguration, error) {
	...
	// if dynamic config is disabled, skip trying to load any checkpoints becasue they won't exist
	if !cc.dynamicConfig {
	...
	// verify the integrity of the checkpoint we just loaded
	err = checkpoint.Verify()
	if err != nil {
		return cc.badRollback(curUID, fmt.Sprintf(status.CurFailVerifyReasonFmt, curUID), fmt.Sprintf("error: %v", err))
	}
...
func (cc *KubeletConfigController) StartSync(client clientset.Interface, nodeName string) {
	if client == nil {
...
func (cc *KubeletConfigController) initialize() error {
...
func (cc *KubeletConfigController) localConfig() *componentconfig.KubeletConfiguration {
...
func (cc *KubeletConfigController) inTrial(trialDur time.Duration) (bool, error) {
	now := time.Now()
...
func (cc *KubeletConfigController) crashLooping(threshold int32) (bool, error) {
...
func (cc *KubeletConfigController) graduateCurrentToLastKnownGood() error {
