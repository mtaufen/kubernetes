...
func (cc *KubeletConfigController) badRollback(uid, reason, detail string) (*componentconfig.KubeletConfiguration, error) {
...
func (cc *KubeletConfigController) lkgRollback(reason string) (*componentconfig.KubeletConfiguration, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%s, error: %v", fmt.Sprintf(status.LkgFailVerifyReasonFmt, lkgUID), err)
	}
