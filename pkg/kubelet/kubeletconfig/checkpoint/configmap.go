...
import (
	...
	"k8s.io/kubernetes/pkg/kubelet/kubeletconfig/checkpoint/hash"
...
// implements Verify for v1/ConfigMap checkpoints
func (c *configMapCheckpoint) Verify() error {
	cm := c.configMap
	name, alg, expectedHash, err := parseCheckpointName(cm.ObjectMeta.Name)
	if err != nil {
		return fmt.Errorf("failed: parse config name %q: %v", err, name)
	}
	actualHash, err := hash.MapStringStringHash(alg, cm.Data)
	if err != nil {
		return fmt.Errorf("failed: hash config data: %v", err)
	}
	if actualHash != expectedHash {
		return fmt.Errorf("failed: verify config data: alg: %q, expected hash: %q, actual hash: %q", alg, expectedHash, actualHash)
	}
	return nil
}
