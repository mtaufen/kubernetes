...
func TestFsStoreLoad(t *testing.T) {
	...
	data, err := utilcodec.EncodeKubeletConfig(expect)
	if err != nil {
...
func TestFsStoreSetCurrent(t *testing.T) {
	...
	const uid = "uid"
	expect := fmt.Sprintf(`{"kind":"NodeConfigSource","apiVersion":"v1","configMap":{"namespace":"namespace","name":"name","uid":"%s","kubeletConfigKey":"kubelet"}}%s`, uid, "\n")
