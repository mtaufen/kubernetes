...
func TestConfigMapCheckpointVerify(t *testing.T) {
	...
	cases := []struct {
		...
		{&configMapCheckpoint{&apiv1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{Name: "empty-data-sha256-e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"},
			Data:       map[string]string{}}}, ""},
		// one key
		{&configMapCheckpoint{&apiv1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{Name: "one-key-sha256-695ceb80f5aa4640897ecc2287dde413dfb595d5f545452bc58bbef5a012fdab"},
			Data:       map[string]string{"one": ""}}}, ""},
		// two keys
		{&configMapCheckpoint{&apiv1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{Name: "two-keys-sha256-2bff03d6249c8a9dc9a1436d087c124741361ccfac6615b81b67afcff5c42431"},
			Data:       map[string]string{"one": "", "two": "2"}}}, ""},
		// hash only
		{&configMapCheckpoint{&apiv1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{Name: "sha256-2bff03d6249c8a9dc9a1436d087c124741361ccfac6615b81b67afcff5c42431"},
			Data:       map[string]string{"one": "", "two": "2"}}}, ""},
		// should use the alg and hash at the end of the name
		{&configMapCheckpoint{&apiv1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{Name: "two-keys-sha256-stuff-sha256-2bff03d6249c8a9dc9a1436d087c124741361ccfac6615b81b67afcff5c42431"},
			Data:       map[string]string{"one": "", "two": "2"}}}, ""},
		// unsupported alg
		{&configMapCheckpoint{&apiv1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{Name: "correct-hash-not-alg-bogus-2bff03d6249c8a9dc9a1436d087c124741361ccfac6615b81b67afcff5c42431"},
			Data:       map[string]string{"one": "", "two": "2"}}}, "not supported"},
		// human name only
		{&configMapCheckpoint{&apiv1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{Name: "humanname"},
			Data:       map[string]string{}}}, "malformed name"},
	}

	for _, c := range cases {
		err := c.cpt.Verify()
		// use SkipRest to log unexected error conditions (positive or negative)
		utiltest.SkipRest(t, c.cpt.configMap.Name, err, c.err)
	}
}
