...
func (tracker *fsTracker) Entry(uid string) (*BadConfigEntry, error) {
	m, err := tracker.load()
	if err != nil {
		return nil, err
	}
...
func (tracker *fsTracker) load() (map[string]BadConfigEntry, error) {
...
func (tracker *fsTracker) save(m map[string]BadConfigEntry) error {
	// encode the map
	data, err := encode(m)
	if err != nil {
		return err
