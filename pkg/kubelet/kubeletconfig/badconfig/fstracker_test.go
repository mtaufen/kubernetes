...
func TestFsTrackerMarkBad(t *testing.T) {
	...
	if entry.Reason != reason {
		t.Errorf("expect BadConfigEntry.Reason %q, but got %q", reason, entry.Reason)
	}

	// the entry's timestamp should be in RFC3339 format
	if err := assertRFC3339(entry.Time); err != nil {
		t.Errorf("expect BadConfigEntry.Time to use RFC3339 format, but got %q, error: %v", entry.Time, err)
	}
...
func TestFsTrackerEntry(t *testing.T) {
	...
	uid := "uid"
	expect := &BadConfigEntry{
		Time:   nowstamp,
		Reason: "reason",
	}
	m := map[string]BadConfigEntry{uid: *expect}
	err = tracker.save(m)
	if err != nil {
...
func TestFsTrackerLoad(t *testing.T) {
	...
	cases := []struct {
		desc   string
		data   []byte
		expect map[string]BadConfigEntry
		err    string
	}{
		// empty file
		{"empty file", []byte(""), map[string]BadConfigEntry{}, ""},
		// empty map
		{"empty map", []byte("{}"), map[string]BadConfigEntry{}, ""},
		// valid json
		{"valid json", []byte(fmt.Sprintf(`{"%s":{"time":"%s","reason":"reason"}}`, uid, nowstamp)),
			map[string]BadConfigEntry{uid: {
				Time:   nowstamp,
				Reason: "reason",
			}}, ""},
		// invalid json
		{"invalid json", []byte(`*`), map[string]BadConfigEntry{}, "failed to unmarshal"},
	}
...
func TestFsTrackerSave(t *testing.T) {
	...
	cases := []struct {
		...
		// empty map
		{"empty map", map[string]BadConfigEntry{}, "{}", ""},
		// 1-entry map
		{"1-entry map",
			map[string]BadConfigEntry{uid: {
				Time:   nowstamp,
...
func TestFsTrackerRoundTrip(t *testing.T) {
	...
	nowstamp := time.Now().Format(time.RFC3339)
	uid := "uid"
	expect := map[string]BadConfigEntry{uid: {
		Time:   nowstamp,
		Reason: "reason",
	}}
