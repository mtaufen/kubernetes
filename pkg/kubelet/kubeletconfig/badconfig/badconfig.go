...
type Tracker interface {
	...
	// Entry returns the BadConfigEntry for `uid` if it exists in the tracker, otherise nil
	Entry(uid string) (*BadConfigEntry, error)
}

// BadConfigEntry describes when a configuration was marked bad and why
...
func markBad(m map[string]BadConfigEntry, uid, reason string) {
	now := time.Now()
...
// getEntry returns the BadConfigEntry for `uid` in `m`, or nil if no such entry exists
func getEntry(m map[string]BadConfigEntry, uid string) *BadConfigEntry {
	entry, ok := m[uid]
	if ok {
		return &entry
	}
...
func encode(m map[string]BadConfigEntry) ([]byte, error) {
...
// decode transforms a []byte into a `map[string]BadConfigEntry`, or returns an error if it can't produce said map
// if `data` is empty, returns an empty map
func decode(data []byte) (map[string]BadConfigEntry, error) {
	// create the map
