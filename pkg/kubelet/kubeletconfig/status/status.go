	...
	s.transact(func() {
		if source != nil {
			...
			if source.ConfigMap != nil {
				// Assigned is validated against the rules for Spec, so we ensure that
				// ResourceVersion remains unset.
				// TODO(#61643): also empty UID when it becomes forbidden in spec
				source.ConfigMap.ResourceVersion = ""
			}
		}
