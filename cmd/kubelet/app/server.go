...
(underspec'd currently) to submit a new manifest.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			// always validate the combination of the local config file and flags, this is the fallback
			// when
			...
			if dynamicConfigDir := kubeletFlags.DynamicConfigDir.Value(); len(dynamicConfigDir) > 0 {
				...
				dynamicKubeletConfig, kubeletConfigController, err = BootstrapKubeletConfigController(kubeletConfig, dynamicConfigDir)
				if err != nil {
					glog.Fatal(err)
				}
