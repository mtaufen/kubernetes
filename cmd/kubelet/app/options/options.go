...
func (f *KubeletFlags) AddFlags(fs *pflag.FlagSet) {
	...
	fs.BoolVar(&f.RunOnce, "runonce", f.RunOnce, "If true, exit after spawning pods from local static pod files or remote urls. Exclusive with --enable-server")
...
func AddKubeletConfigFlags(mainfs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.StringVar(&c.PodPath, "pod-manifest-path", c.PodPath, "Path to the directory containing static pod files to run, or the path to a single static pod file. Files starting with dots will be ignored.")
