...
func (f *KubeletFlags) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringSliceVar(&f.APIServerList, "api-servers", f.APIServerList, "List of Kubernetes API servers for publishing events, and reading pods and services. (ip:port), comma separated.")
	...
	fs.StringVar(&f.NodeConfigDir, "node-config-dir", f.NodeConfigDir, "The Kubelet's node configuration controller will use this directory for checkpointing downloaded configurations, finding the (optional) init configuration, and recording miscellaneous information. The path may be absolute or relative; relative paths are under the Kubelet's RootDirectory.")
}
