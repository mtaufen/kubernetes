...
var (
	etcdLocalLongDesc = fmt.Sprintf(normalizer.LongDesc(`
		Generates the static Pod file for a local, single-node etcd instance and saves it to %s file.
	...
	etcdLocalExample = normalizer.Examples(`
		# Generates the static Pod file for etcd, functionally
		# equivalent to what generated by kubeadm init.
		kubeadm alpha phase etcd local

		#  Generates the static Pod file for etcd.
...
func NewCmdEtcd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "etcd",
		Short: "Generates static Pod file for etcd.",
		Long:  cmdutil.MacroCommandLongDescription,
	}

	podPath := kubeadmconstants.GetStaticPodDirectory()
	cmd.AddCommand(getEtcdSubCommands(podPath, "")...)
	return cmd
}
...
func getEtcdSubCommands(outDir, defaultKubernetesVersion string) []*cobra.Command {
	...
	properties := struct {
		...
		use:      "local",
		short:    "Generates the static Pod file for a local, single-node etcd instance",
		long:     etcdLocalLongDesc,
		examples: etcdLocalExample,
		cmdFunc:  etcdphase.CreateLocalEtcdStaticPodFile,
	}
