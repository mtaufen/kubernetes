...
var (
	allControlplaneLongDesc = normalizer.LongDesc(`
		Generates all static Pod files necessary to establish the control plane.
		` + cmdutil.AlphaDisclaimer)

	allControlplaneExample = normalizer.Examples(`
		# Generates all static Pod files for control plane components,
		# functionally equivalent to what generated by kubeadm init.
		kubeadm alpha phase controlplane all

		# Generates all static Pod files using options read from a configuration file.
	...
	apiServerControlplaneLongDesc = fmt.Sprintf(normalizer.LongDesc(`
		Generates the static Pod file for the API server and saves it into %s file.
	...
	controllerManagerControlplaneLongDesc = fmt.Sprintf(normalizer.LongDesc(`
		Generates the static Pod file for the controller-manager and saves it into %s file.
	...
	schedulerControlplaneLongDesc = fmt.Sprintf(normalizer.LongDesc(`
		Generates the static Pod file for the scheduler and saves it into %s file.
...
func NewCmdControlplane() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "controlplane",
		Short: "Generates all static Pod files necessary to establish the control plane",
		Long:  cmdutil.MacroCommandLongDescription,
	}

	podPath := kubeadmconstants.GetStaticPodDirectory()
	cmd.AddCommand(getControlPlaneSubCommands(podPath, "")...)
	return cmd
}
...
func getControlPlaneSubCommands(outDir, defaultKubernetesVersion string) []*cobra.Command {
	...
	subCmdProperties := []struct {
	...
	}{
		{
			...
			cmdFunc:  controlplanephase.CreateInitStaticPodFiles,
		},
		{
			use:     "apiserver",
			short:   "Generates the API server static Pod.",
			long:    apiServerControlplaneLongDesc,
			cmdFunc: controlplanephase.CreateAPIServerStaticPodFile,
		},
		{
			use:     "controller-manager",
			short:   "Generates the controller-manager static Pod.",
			long:    controllerManagerControlplaneLongDesc,
			cmdFunc: controlplanephase.CreateControllerManagerStaticPodFile,
		},
		{
			use:     "scheduler",
			short:   "Generates the scheduler static Pod.",
			long:    schedulerControlplaneLongDesc,
			cmdFunc: controlplanephase.CreateSchedulerStaticPodFile,
		},
	}
