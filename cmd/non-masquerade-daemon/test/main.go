...
func check(ip string, pip string, nip string) error {
	...
	if rip != pip {
		if rip == nip {
			...
			return fmt.Errorf("Returned ip %q != my Pod ip %q or my Node ip %q - SNAT through unexpected ip (likely a secondary interface, e.g. cbr0 instead of eth0)", rip, pip, nip)
		}
	}
	return nil
}
